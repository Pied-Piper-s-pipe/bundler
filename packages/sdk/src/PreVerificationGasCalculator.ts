...
export const ChainConfigs: {
  [key: number]: PreVerificationGasCalculatorConfig;
} = {
  1: MainnetConfig,
  1337: MainnetConfig,
  211101: MainnetConfig,
};
...
export class PreVerificationGasCalculator {
  ...
  validatePreVerificationGas(userOp: UserOperation): {
    ...
    return {
      ...
      isPreVerificationGasValid:
        minRequiredPreVerificationGas <=
        parseInt((userOp.preVerificationGas as any).toString()),
    };
  }
   ...
   */
  estimatePreVerificationGas(userOp: Partial<UserOperation>): number {
  ...
  _calculate(userOp: UserOperation): number {
    ...
    const callDataCost = packedUserOp
      .map((x) =>
        x === 0 ? this.config.zeroByteGasCost : this.config.nonZeroByteGasCost
      )
      .reduce((sum, x) => sum + x);
    const userOpDataWordsOverhead =
      userOpWordsLength * this.config.perUserOpWordGasOverhead;

    const userOpSpecificOverhead =
      callDataCost + userOpDataWordsOverhead + this.config.perUserOpGasOverhead;
    const userOpShareOfBundleCost =
      this.config.fixedGasOverhead / this.config.expectedBundleSize;
  ...
  _fillUserOpWithDummyData(userOp: Partial<UserOperation>): UserOperation {
    ...
    filledUserOp.signature =
      filledUserOp.signature ??
      hexlify(Buffer.alloc(this.config.estimationSignatureSize, 0xff));
    filledUserOp.paymasterData =
      filledUserOp.paymasterData ??
      hexlify(Buffer.alloc(this.config.estimationPaymasterDataSize, 0xff));
    return filledUserOp;
  }
}
